...
def main():
    ...
    if not os.path.exists(cache_path):
        print("Cache file not found. Run generate_cache.py first.")
        return
    ...
    # Run solver with optimal settings (5 seeds, 500 SA iterations)
    # This will take ~19 minutes and run in the background
    print("Fixing 804_stop.txt in cache (Optimal Alg 2: 5 seeds, 500 SA)...")
    st = time.time()
    vn, sm, nh, td, rtd, routes = solver.solve(
        algo_type=2, initial_vehicles=5, sa_iterations=500, optimize_overlap=False, num_seeds=5
    )
    ct = time.time() - st
    print(f"  Result: Vn={vn}, Sm={sm:.1f}, Nh={nh}, TD={td:.1f} in {ct:.1f}s")
    
    # Cache for both Hybrid and Paper since the optimal routes are identical
    cache[f"{f}_alg2_hybrid"] = {
        "vn": vn,
        "sm": sm,
        "nh": nh,
        "td": td,
    ...
    print("Successfully updated 804_stop.txt Alg 2 entries in cache!")
